...
def get_old_readings(wb, room, start_row):
    ws = wb[room]
    old_e = ws.cell(start_row + 1, 9).value  # I row is "new"; H is previous
    old_w = ws.cell(start_row + 2, 9).value
    return old_e, old_w
...
def prepare_review(wb, extraction):
    ...
    for rr in extraction.rooms:
        ...
        old_e = ws.cell(row + 1, 9).value if row else None
        old_w = ws.cell(row + 2, 9).value if row else None
...
def main():
    ...
    if st.button("▶️ BẮT ĐẦU ĐỌC ẢNH", type="primary", disabled=not (images and excel)):
        try:
            ...
            if missing_rooms:
                st.warning("Chưa đọc thấy các phòng: " + ", ".join(missing_rooms))
    ...
    if "review" in st.session_state:
        ...
        if missing_rooms:
            st.warning("Chưa có dữ liệu cho: " + ", ".join(missing_rooms))

        can_confirm = len(bad) == 0 and not missing_rooms
